...
//! 提供不同使用场景的配置示例
//!
//! 所有模板均使用原始字符串（raw string）书写：
//! - 避免普通字符串的 `\` 续行吞掉行首缩进（曾导致 `css_variables` 子项变成顶层键）；
//! - 正则中的反斜杠按字面保留。包含 `\s`、`\d` 等序列的值在 YAML 中
//!   必须使用单引号（单引号 YAML 标量不做转义处理），双引号会把 `\s` 当作
//!   非法转义导致解析失败。
...
# 章节匹配规则（更宽松）。含 \s、\S 的正则必须用单引号，
# 双引号 YAML 会把 \s 当作非法转义导致加载失败。
...
# CSS 变量（注意子项必须缩进，作为 css_variables 的子键）
css_variables:
